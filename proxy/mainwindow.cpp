...
void MainWindow::processConnection()
{
    QTcpSocket *clientSocket = server->nextPendingConnection();
       if(isListening == true)
       {
           ...
           ProxyThread *proxyThread = new ProxyThread(clientSocket,Id++,isIntercepting, this);
           connect(this, &MainWindow::setNextPressed, proxyThread, &ProxyThread::onNextPressed);
           connect(proxyThread, &ProxyThread::SetListening, this, &MainWindow::handleSetListening);
...
void MainWindow::handleSetListening()
{
...
bool MainWindow::findIP(const QString &ip){

    for (auto i = trafficData.begin(); i != trafficData.end(); ++i)
    {
...
{
    QString currentDateTime = QDateTime::currentDateTime().toString(Qt::ISODate);
    const QString filePath = currentDateTime;
    QFile file(filePath);
...
void MainWindow::on_pushButton_clicked()
{
    ...
    QString bip = ui->IpAdress->toPlainText();
    trafficData[this->Id].addIp(bip);
    Id++;
    qDebug()<<" ";
    qDebug()<< " ";
}
